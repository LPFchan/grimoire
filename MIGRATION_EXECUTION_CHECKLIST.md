...
- 2026-05-15 isolated native-canary result: `dflash-native-qwen3.6-27B-canary` still fails at control-plane validation on the isolated container with `409 Conflict` because the configured draft GGUF reports architecture `qwen3`, while the current native validator requires `dflash-draft` or `qwen35-dflash-draft`. Native hardware decode remains blocked before runtime.

...
- Canonical target repo: `TheTom/llama-cpp-turboquant` via `https://github.com/TheTom/llama-cpp-turboquant.git` (`feature-turboquant-kv-cache-b9079-69d8e4b` branch), local snapshot at `tmp/spec-analysis/thetom-shallow/`. Current SHA: `69d8e4be47243e83b3d0d71e932bc7aa61c644dc`.
- DFlash source refs: `spiritbuun/buun-llama-cpp` via `https://github.com/spiritbuun/buun-llama-cpp.git`, local snapshot at `tmp/spec-analysis/buun-shallow/` (SHA `853eebdd02c2db4baf7bf781adadee6e7ce1d44e`). `Anbeeld/beellama.cpp` via `https://github.com/Anbeeld/beellama.cpp.git`, local snapshot at `tmp/spec-analysis/bee-shallow/` (SHA `633cd34fb6df72ad88a74e9320dad03ddd788fb1`). `ggml-org/llama.cpp` PR 22105 via `https://github.com/ggml-org/llama.cpp.git`, local snapshot at `tmp/spec-analysis/ggml-pr22105/` (SHA `320a6a44a5b1de6a074ba781e65f5fd79fb4051a`).
- PFlash upstream baseline: `Luce-Org/lucebox-hub` via `https://github.com/Luce-Org/lucebox-hub.git`, local snapshot at `tmp/spec-analysis/lucebox-hub/` -- pinned to the `de31881` file baseline for `pflash_daemon`, `qwen3_drafter`, and `SPEC_PREFILL`. Current SHA: `e5347801719ad7d45a3d7bd096e9e57778ce23ea`.
...
- 2026-05-16 Phase 0 fresh-state GPU 1 baseline (isolated `http://127.0.0.1:9002`, fresh state volume, live `9001` untouched): before first request GPU 1 memory was `1 MiB`, tmpfs snapshots were `0 MiB`, and `/var/lib/grimoire/snapshot_swap` did not yet exist. Cold short served DFlash decode on `dflash-pflash-qwen3.6-27B` (`Say hello in five words.`, `max_tokens=16`) measured TTFT about `5213.8 ms`, decode about `292.8 ms`, decode throughput about `54.64 tok/s`, prompt tokens `16`, completion tokens `16`. A fresh short real-session `/history` probe then measured turn-1 TTFT about `683.6 ms`, decode about `473.8 ms`, decode throughput about `33.77 tok/s`, and turn-2 TTFT about `801.0 ms`, decode about `117.3 ms`, decode throughput about `136.41 tok/s`, with observed `turn1/turn2` TTFT speedup about `0.85x` on this tiny prompt. After that short session, GPU 1 memory was about `23976 MiB`, tmpfs snapshots were still `0 MiB`, disk snapshot swap was about `1 MiB`, and the fresh disk snapshot tree only contained `session-kv.json` (`189 B`) and `swap-manifest.json` (`12 B`).
- 2026-05-16 Phase 0 five-run short medians on isolated GPU 1: across five sequential short served-DFlash runs, measured medians were about `816.4 ms` TTFT and `87.86 tok/s` decode for the one-turn short prompt; about `676.3 ms` TTFT and `33.25 tok/s` decode for short session turn 1; and about `845.0 ms` TTFT and `37.91 tok/s` decode for short session turn 2. These runs were not perfectly stable: one basic run ended with a zero-token OOM-style SSE error, one turn-1 run reported `dflash daemon not running`, one turn-2 run reported `dflash daemon not running`, and one turn-2 run reported the familiar zero-token VRAM error. The medians are therefore usable as an initial operational baseline, but not yet a final stable-release baseline.
- 2026-05-16 Phase 0 preserved-PFlash compression baseline on isolated GPU 1: the earlier single-message probe against `pflash-qwen3.6-27B` crossed neither threshold nor compressible middle-region requirements, so it completed as a wall-clock-only datapoint (`20,000` chars, `5,878` prompt tokens, about `14.8 s`, `16` completion tokens). A later threshold-crossing single-message probe reached `40,746` raw tokens in about `46.5 s` and surfaced `pflash debug: fired=False raw=40,746 compressed=40,746`, which matches current block-protection behavior because a one-message prompt leaves no compressible middle span after head/tail protection. A threshold-crossing multi-turn probe then produced the first explicit live ratio: `13` messages / about `150,136` chars, `pflash debug: fired=True raw=43,584 compressed=23,296 ratio=1.87x`, API-visible effective prompt tokens `23,362`, and about `21.0 s` wall-clock with GPU 1 VRAM steady around `20.4 GiB`. This is the first trustworthy preserved-PFlash raw-vs-compressed baseline from live gateway instrumentation on the isolated container.
- 2026-05-16 Phase 0 bounded snapshot-growth mini-run on isolated GPU 1: starting from tmpfs `0 MiB` and disk snapshot swap `1054 MiB` on the reused fresh container volume, a six-turn capped short session reached three successful turns before failing on turn 4 with the familiar zero-token VRAM error. Observed growth during the run was: turn 1 (`23` prompt tokens) TTFT about `5393.4 ms`, tmpfs about `351 MiB`, disk about `968 MiB`; turn 2 (`47` prompt tokens) TTFT about `796.7 ms`, tmpfs about `352 MiB`, disk about `933 MiB`; turn 3 (`71` prompt tokens) TTFT about `898.4 ms`, tmpfs back to `0 MiB`, disk about `703 MiB`; turn 4 failed with `DFlash generation failed: model produced zero tokens` and left tmpfs at `0 MiB` and disk at `703 MiB`. This gives an initial bounded-growth datapoint and another concrete small-session failure envelope on current hardware.
- 2026-05-16 Phase 0 fresh-state turn-1 failure envelope on isolated GPU 1: a stricter disposable-container sweep (one brand-new container + brand-new state volume per candidate on port `9003`) showed the reused-state restore sweep was misleading, but also exposed a new fresh-state wall on DFlash turn 1 itself. A hand-written tiny real session still passed cleanly on a fresh sibling container (`16` prompt tokens on turn 1, `49` prompt tokens on turn 2, TTFT about `5315.1 ms` then `980.7 ms`), confirming no blanket fresh-start regression. However, a fixture-built single-message prompt of `1000` chars (`288` prompt tokens) failed on turn 1 with `DFlash generation failed: model produced zero tokens`, and container logs showed prefill finishing (`[prefill] 288 tokens in 0.96 s`) before dying at `cache migration: ggml_backend_alloc_ctx_tensors failed for rollback cache`. A synthetic fresh-state sweep using simple repeated text narrowed the threshold: turn 1 succeeded at `64`, `127`, and `193` prompt tokens, but failed at `256` and `289` prompt tokens with the same rollback-cache allocation error after successful prefill. This establishes that the current 3090 configuration has a fresh-state turn-1 rollback/cache migration failure band starting somewhere between about `193` and `256` prompt tokens for single-message prompts, independent of reused snapshot state.
- 2026-05-16 rollback-cache mitigation verification on isolated GPU 1: source inspection showed `lucebox/dflash/src/qwen35_target_graph.cpp` allocated rollback `ssm_intermediate` tensors as `GGML_TYPE_F16` during `migrate_prefill_cache(...)`, while the full-cache creation path already used `GGML_TYPE_Q8_0`. After patching the migration path to use `Q8_0`, rebuilding the native `dflash` daemon in a CUDA-capable build container, and bind-mounting the patched `/opt/dflash` into a disposable fresh-state container, the previously failing fresh-state turn-1 cases under the original model config (`budget=18`) now passed: `256` prompt tokens completed with TTFT about `6411.1 ms`, prefill about `0.87 s`, migrate about `121.97 ms`, and decode about `70.44 tok/s`; `289` prompt tokens completed with TTFT about `6070.0 ms`, prefill about `1.00 s`, migrate about `119.16 ms`, and decode about `69.39 tok/s`. This confirms the earlier rollback-cache wall was materially caused by the migration-path tensor type mismatch and that the native patch moves the fresh-state failure threshold upward on real 3090 hardware.
- 2026-05-16 patched fresh-state next failure band on isolated GPU 1: continuing the same disposable-container method with the patched `/opt/dflash` and the original served config (`budget=18`) moved the fresh-state turn-1 ceiling further upward but did not eliminate the hardware wall. A synthetic single-message prompt at `319` prompt tokens passed with TTFT about `6190.7 ms`, prefill about `1.06 s`, migrate about `156.88 ms`, and decode about `70.11 tok/s`. Fresh-state turn 1 then failed again above that level: `385` prompt tokens failed after prefill (`1.30 s`) and migrate (`124.62 ms`) with the usual zero-token client error, while `448` and `511` prompt tokens failed with the familiar `cache migration: ggml_backend_alloc_ctx_tensors failed for rollback cache` line after successful prefill. This places the current patched fresh-state turn-1 failure band somewhere between about `319` and `385` prompt tokens for simple single-message prompts on the current 3090 configuration.
- 2026-05-16 Phase 0 remaining baseline gaps: no trustworthy cold moderate/long-prompt restore baseline is recorded yet because served DFlash session turns still hit GPU-memory failures on the current 3090 configuration; and no final stable five-run median set is recorded yet because even short runs still show intermittent daemon/OOM failures.
...
- Current software-only status: Grimoire-side launch wiring and normal-llama runtime-path isolation are covered locally by `tests/test_drop_in_blockers.py`, including normal llama startup env construction without `/opt/dflash` in `LD_LIBRARY_PATH`, park-model shim scoping via `LD_PRELOAD`, and fail-closed prerequisite validation for llama-side PFlash models. The runtime image still carries temporary migration debt under `/opt/dflash` for preserved PFlash components and the legacy DFlash daemon bundle; final removal remains a later cutover gate, not a completed Phase 1 outcome.
...
- 2026-05-15 isolated GPU 1 hardware check: launched a parallel validation container from `/home/yeowool/grimoire-refactor` on `http://127.0.0.1:9002` with only physical GPU 1 exposed, separate `/var/lib/grimoire` state, separate tmpfs snapshot/slot dirs, and refactor code/config bind-mounted in, leaving the live `9001` container untouched. Served `dflash-pflash-qwen3.6-27B` basic smoke passed on the isolated container (`tests/test_e2e_smoke.py::DFlashSmokeTests::test_01_basic_chat_completion`), proving real GPU-backed decode on the refactor path. A real short `/history` two-turn manual probe also passed (`turn1` about `5.64s`, `turn2` about `0.87s`), confirming the short-prompt session restore path can work on GPU 1.
- 2026-05-15 isolated GPU 1 hardware blocker: the served DFlash restore path still fails for moderate/longer prompts on a 3090 with the current served config (`budget=18`, current rollback cache behavior). Real first-turn failures reproduced with SSE error payloads / daemon logs showing `DFlash generation failed: model produced zero tokens` plus CUDA OOM during verify/rollback cache allocation, including `ddtree verify build failed`, `cache migration: ggml_backend_alloc_ctx_tensors failed for rollback cache`, and allocations around `1.2-1.6 GiB` after prefill on prompts that tokenized to roughly `312`, `580`, and `3786` tokens in isolated runs. This means Phase 2/3 hardware sign-off is still blocked for the served restore envelope at current settings.
- 2026-05-15 isolated native-canary result: `dflash-native-qwen3.6-27B-canary` originally failed at control-plane validation because the config pointed to `Qwen3-0.6B-BF16.gguf` (arch `qwen3`). This was fixed on 2026-05-16 by switching the config to `Qwen3.5-0.8B-Q8_0.gguf` (arch `qwen35`) and adding `qwen35` to `_NATIVE_DFLASH_ALLOWED_ARCHES` in `registry.py`. Native hardware decode remains blocked at runtime pending the TheTom binary's ability to load a plain `qwen35` GGUF as a DFlash draft.
- Current fresh-review fix status: the native GGUF draft validator in `src/grimoire/registry.py` now mirrors the C++ loader contract more closely, performs streaming GGUF parsing instead of full-file reads, enforces the compiled `block_size=16` / `n_target_layers=5` expectations, validates required draft hparams, and rejects missing per-layer draft tensors. The served `.safetensors` draft remains pinned on `dflash-pflash-qwen3.6-27B`; GGUF remains canary-only until hardware decode validation proves the cutover.
...
- Current software-only status: Grimoire-side persistence hardening is covered locally by `tests/test_dflash.py` and `tests/test_drop_in_blockers.py`, including stale session/prefix invalidation, zero-token cleanup, disk TTL/budget cleanup, model-scoped snapshot RAM paths, async mirror invalidation on `discard()` / `clear()`, stray disk-file cleanup, and pending-mirror flush on DFlash shutdown. Real restart-resilience and soak validation remain hardware-gated.
- 2026-05-15 isolated GPU 1 persistence note: a real hardware leak was found and fixed in `src/grimoire/proxy/dflash.py` during isolated validation. One-off served DFlash requests were taking a transient staging snapshot without persisting or freeing it, which stranded daemon slot state and caused a second short request to OOM on GPU 1. After fixing that path, two sequential short non-session DFlash requests succeeded on the isolated container. The remaining hardware blocker is therefore narrowed to rollback/restore memory pressure on longer session turns, not the earlier transient staging-slot leak.
...
- Current software-only status: Grimoire-side preserved-PFlash hardening is covered locally by `tests/test_llama_proxy.py` and `tests/test_drop_in_blockers.py`, including exception-safe park/unpark, validated-`conversation_id` slot naming, restore/save parity for restored warm turns even when compression does not fire, and the shim-side FIFO control/ack open pattern needed by the existing Python park/unpark caller. Live GPU validation for real `pflash_daemon`, `/slots/0` persistence, and park/unpark behavior remains open.
- 2026-05-15 harness update: live smoke/stress harnesses now create real `/history` conversations before asserting restore/continuity behavior, support configurable long-prompt windows via `GRIMOIRE_LONG_PROMPT_MIN_CHARS` / `GRIMOIRE_LONG_PROMPT_MAX_CHARS`, accept isolated container metrics via `GRIMOIRE_SMOKE_CONTAINER`, accept host GPU selection in `tests/test_pflash_pipeline.py` via `GPU_INDEX`, and surface backend SSE error messages instead of failing only with empty assistant text. These harness fixes were necessary to distinguish genuine GPU OOM/runtime failures from stale test assumptions.
- Current fresh-review fix status: llama-side preserved-PFlash isolation now also covers per-model shim FIFO bases, model-scoped `.kv` slot filenames, and per-model serialization of `/slots/0` restore -> generate -> save so aliases sharing the same backend slot path cannot collide in software-only tests.

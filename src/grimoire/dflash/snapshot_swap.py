...
class SnapshotSwap:
    ...
    def reserve_slot(self, daemon, key: bytes) -> Optional[int]:
        ...
        # If this key is on disk, load it into a free slot.
        if key in self.disk:
            used = set(self.vram.values())
            for slot in range(8):
                if slot not in used:
                    self._load_from_disk(daemon, key, slot)
                    self.disk.pop(key)
                    self.vram[key] = slot
                    self._save_manifest()
                    return slot
            # No free slot — evict LRU first, then retry.
            ...
            self._save_manifest()
            self._load_from_disk(daemon, key, lru_slot)
            self.disk.pop(key)
            self.vram[key] = lru_slot
            self._save_manifest()
            return lru_slot

        # New key — find a free slot or evict LRU.
        ...
        # All 8 slots in use (shouldn't happen with 8 slots max).

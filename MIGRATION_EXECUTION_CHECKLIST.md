...
- Canonical target repo: working fork of `TheTom/llama-cpp-turboquant` using `tmp/spec-analysis/thetom-shallow/` as the local snapshot. Record the exact SHA here before Phase 1.
- DFlash source refs: `tmp/spec-analysis/buun-shallow/`, `tmp/spec-analysis/bee-shallow/`, and `tmp/spec-analysis/ggml-pr22105/`. Record exact SHAs here before Phase 1.
- PFlash upstream baseline: `tmp/spec-analysis/lucebox-hub/` pinned to the `de31881` file baseline for `pflash_daemon`, `qwen3_drafter`, and `SPEC_PREFILL`.
